...
const SearchLocationBar= () => {
  ...
  const debouncedSetQuery = useRef(
    debounce((value) => setsearchQuery(value),300)).current;
    ...
    try {
      if(currentUser?.interests && currentUser?.location){
        await updateLocation({
          ...
          interests: currrentUser.interests,
        });
      }
      const slug = createLocationSlug(city,state);
      router.push(`/explore/${slug}`);
    } catch (error) {
      console.log("falied to update location :" , error);
    }
  }
  return (
   <div className='flex items-center'>
    <div className='relative flex w-full' ref={searchRef}>
      <div className='flex-1'>
        ...
        className="pl-10 w-full h-9 rounder-none rounded-1-md"/>
      </div>
            ...
            <div className='py-2'>
              <p className='px-4 py-2 text-xs font-semibold text-muted-foreground'>
                ...
                {searchResults.map((event)=>(
                  ...
                  className='w-full px-3 py-3 hover:bg-muted/50 text-left transition-colors'>
